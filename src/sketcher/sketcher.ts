...
export class Sketcher {
    ...
    public onModelSet() {
        ...
        if (!self.controller || !self.controller.canvas || !self.sourceModel || !self.controller.model || !self.controller.renderer) {
            return;
        }
    ...
    public drawNextElement(sketcher: Sketcher) {
        this.timerHandle = undefined;
        if (!sketcher.controller || !sketcher.controller.canvas || !sketcher.drawModel ||
            !sketcher.controller.renderer || !sketcher.context || !sketcher.controller.model ||
        ...
        if (sketcher.elementIndex >= sketcher.elementCount || sketcher.elementIndex < 0) {
            if (sketcher.passIndex === 1) {
                if (sketcher.repeat) {
                    ...
                    sketcher.timerHandle = setTimeout(
                        this.drawNextElement,
                        sketcher.repeatDelay,
                        sketcher
                    );
                }
                else {
