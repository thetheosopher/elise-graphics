...
export class PathElement extends ElementBase implements IPointContainer {
   ...
   */
  public setWinding(winding: WindingMode): PathElement {
   ...
   */
  set commands(newValue: string | undefined) {
    if (newValue) {
   ...
   * @returns This path element
   */
  public setCommands(commandString: string): PathElement {
  ...
  public draw(c: CanvasRenderingContext2D): void {
    ...
    if (FillFactory.setElementFill(c, this as ElementBase)) {
    ...
    if (model.setElementStroke(c, this as ElementBase)) {
      c.stroke();
    }
    c.restore();
  }
  ...
  public translate(offsetX: number, offsetY: number) {
    ...
    for (let i = 0; i < cl; i++) {
      ...
      if (command.charAt(0) === 'm') {
        this._commands[i] =
          'm' + Point.translate(Point.parse(command.substring(1, command.length)), offsetX, offsetY).toString();
      } else if (command.charAt(0) === 'l') {
        this._commands[i] =
          'l' + Point.translate(Point.parse(command.substring(1, command.length)), offsetX, offsetY).toString();
      } else if (command.charAt(0) === 'c') {
        const parts = command.substring(1, command.length).split(',');
   ...
   * @returns This path element
   */
  public setLocation(pointSource: string | Point): ElementBase {
    const b = this.getBounds();
    if (!b) {
      return this as ElementBase;
    }
    let pt: Point;
   ...
   * @returns This path element
   */
  public add(command: string): PathElement {
    if (!this._commands) {
      this._commands = [];
    }
