...
export class EllipseElement extends ElementBase {
  ...
  public draw(c: CanvasRenderingContext2D): void {
    ...
    if (!model || !this._center || !this.radiusX || !this.radiusY) {
      return;
    }
    c.save();
    if (this.transform) {
  ...
  public toString(): string {
    if (!this.type || !this._center || !this.radiusX || !this.radiusX) {
      return super.toString();
    }
    return (
      this.type + ' - c(' + this._center.x + ',' + this._center.y + '), rx:' + this.radiusX + ', ry:' + this.radiusY
    );
  }
   ...
   * @returns This ellipse element
   */
  public setLocation(point: Point): EllipseElement {
    if (this.radiusX && this.radiusY) {
   ...
   * @returns This ellipse element
   */
  public setSize(size: Size): EllipseElement {
    if (!this.radiusX || !this.radiusY || !this._center) {
      return this;
    }
    const x = this._center.x - this.radiusX;
